...
class PixArtTransformer2DModelX(ModelMixin, ConfigMixin):
    r"""
    ...
    def forward(
        ...
        if cross_attention_kwargs is not None:
            # breakpoint()
            i = cross_attention_kwargs['kwargs']['i']
            anchor_token = self.anchor_token[i]
            # breakpoint()
            anchor_token = anchor_token.expand(hidden_states.size(0), -1, -1)
            # breakpoint()
            hidden_states = torch.cat((hidden_states, anchor_token), dim = 1)
        # hidden_states.requires_grad = True
        # anchor_token = torch.zeros(2,1,1152).to(hidden_states.device)
        # hidden_states = torch.cat((hidden_states, anchor_token), dim = 1)
        # breakpoint()
